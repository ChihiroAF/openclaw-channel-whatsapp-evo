...
import type { ResolvedEvoAccount } from "./accounts.js";
...
import type { EvoTextMessage } from "./payload.js";

export type DispatchWabaEvoInboundParams = {
  account: ResolvedEvoAccount;
  cfg: unknown;
  channelRuntime: PluginRuntime["channel"];
  logger: ChannelLogger;
  message: EvoTextMessage;
  /**
   * controller 转发的 `X-Evo-Instance` 头原值。
   * 它必须**随请求透传到这里**（而不是从 payload 里解析）：v1 payload 的 `instance` 是对象、
   * v2 是字符串，形态不稳定；头是 controller 按映射表下发的，权威且稳定。
   */
...
export async function dispatchWabaEvoInbound(params: DispatchWabaEvoInboundParams): Promise<void> {
  ...
  const ingress = await resolveStableChannelMessageIngress({
    ...
    dmPolicy: account.dmPolicy as "open" | "allowlist" | "pairing",
  ...
  if (ingress.ingress.admission !== "dispatch") {
    ...
    logger.info?.(
      `[whatsapp-evo] inbound blocked by channel ingress policy (admission=${ingress.ingress.admission}, ` +
  ...
  const dispatch = await channelRuntime.inbound.dispatch({
    ...
    delivery: {
      ...
      onError: (error) => {
        logger.error?.(
          `[whatsapp-evo] agent turn failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      },
    },
    replyPipeline: {},
