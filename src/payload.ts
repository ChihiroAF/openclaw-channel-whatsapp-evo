...
import {
  EVO_MEDIA_MESSAGE_TYPES,
  EVO_MESSAGE_TYPE_CONVERSATION,
  EVO_MESSAGE_TYPE_EXTENDED_TEXT,
  EVO_MESSAGE_UPSERT_EVENT_NORMALIZED,
  WHATSAPP_BROADCAST_JID_SUFFIX,
  WHATSAPP_GROUP_JID_SUFFIX,
} from "./constants.js";

/** 跳过的原因（用于日志归类，便于联调时定位是哪一类没认出来） */
export type EvoSkipReason =
  | "empty_body"
  | "invalid_json"
  | "non_message_event"
  | "from_me"
...
/** 归一化后的文本入站消息 */
export interface EvoTextMessage {
  /** EVO 消息 ID（wamid），可能缺失，缺失时为空串 */
  wamid: string;
  /** 原始 remoteJid，仅用于日志排查 */
  remoteJid: string;
  /** 纯数字国际格式（不含 +），即回复目标 */
  senderNumber: string;
  /** 客户昵称，可能为空 */
  ...
  /** 文本正文（已 trim 判定，原样返回） */
  text: string;
  /** 秒级时间戳，取不到时为 undefined */
  timestampSeconds?: number;
}

export type EvoParseResult =
  | { kind: "text"; message: EvoTextMessage }
  | { kind: "media"; wamid: string; senderNumber: string; mediaType: string }
  | { kind: "skip"; reason: EvoSkipReason; wamid?: string };
 ...
 */
export function normalizeEvoEventName(event: unknown): string {
...
/** 是否为「收到新消息」事件 */
export function isMessageUpsertEvent(event: unknown): boolean {
 ...
 */
export function normalizeWhatsAppNumber(remoteJid: string): string {
...
export function isMediaMessageType(messageType: string): boolean {
  return (EVO_MEDIA_MESSAGE_TYPES as readonly string[]).includes(messageType);
}
...
export function extractText(message: Record<string, unknown> | undefined): string | undefined {
  if (message === undefined) {
    return undefined;
  }
 ...
 */
export function extractTimestampSeconds(value: unknown): number | undefined {
...
function toUtf8(rawBody: string | Uint8Array): string {
  return typeof rawBody === "string" ? rawBody : Buffer.from(rawBody).toString("utf8");
}

function skip(reason: EvoSkipReason, wamid?: string): EvoParseResult {
 ...
 */
export function parseEvoWebhook(rawBody: string | Uint8Array | undefined | null): EvoParseResult {
