...
export class EvoClientError extends Error {
    ...
    timedOut;
    constructor(message, httpStatus, responseExcerpt, timedOut = false) {
        super(message);
 ...
 */
export function normalizeEvoBaseUrl(raw) {
...
export function buildSendTextBody(number, text) {
    return { number, text };
}
/**
 * 从 EVO 响应体里尽力取字段。
 ...
 */
export function extractEvoMessageId(responseBody) {
    ...
    for (const field of ["id", "messageId"]) {
...
export function extractEvoStatus(responseBody) {
    const root = asRecord(responseBody);
...
export function toEvoNumber(raw) {
...
export class EvoClient {
    ...
    async sendText(params) {
        ...
        try {
            ...
            throw new EvoClientError(timedOut
                ? `EVO sendText timed out after ${this.timeoutMs}ms`
                : `EVO sendText request failed: ${reason}`, 0, "", timedOut);
        }
        const rawBody = await response.text();
        if (!response.ok) {
            throw new EvoClientError(`EVO sendText rejected with HTTP ${response.status}`, response.status, truncate(rawBody));
        }
        let parsed;
        try {
